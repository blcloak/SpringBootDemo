...
public class JEBaseAPI {
	...
	public void openDatabase() {
		try {
			...
			// CheckMethods.PrintDebugMessage(dbName + "数据库中的数据个数: " +
			// myDatabase.count());
			/*
		...
		try {
			...
			try {
				...
				if (res == OperationStatus.SUCCESS) {
					// CheckMethods.PrintDebugMessage("向数据库" + dbName + "中写入:" +
					// key + "," + value);
					return true;
				...
				txn.abort();
				// CheckMethods.PrintInfoMessage("向数据库" + dbName + "中写入:" + key
				// + "," + value + "出现lock异常");
	...
	public boolean deleteFromDatabase(String key) {
		...
		for (int i = 0; i < 3; i++) {
			...
			try {
				...
				txn.commit();
				if (res == OperationStatus.SUCCESS) {
					// CheckMethods.PrintDebugMessage("从数据库" + dbName + "中删除:" +
					// key);
					success = true;
					return success;
				} else if (res == OperationStatus.KEYEMPTY) {
					// CheckMethods.PrintDebugMessage("没有从数据库" + dbName + "中找到:"
					// + key + "。无法删除");
				} else {
					// CheckMethods.PrintDebugMessage("删除操作失败，由于" +
					// res.toString());
				}
				return false;
		...
		try {
			...
			try {
				...
				if (res == OperationStatus.SUCCESS) {
					...
					// CheckMethods.PrintDebugMessage("从数据库" + dbName + "中读取:" +
					// key + "," + foundData);
					return foundData;
				} else {
					// CheckMethods.PrintDebugMessage("No record found for key
					// '" + key + "'.");
					return "";
				}
			} catch (LockConflictException lockConflict) {
				txn.abort();
				// CheckMethods.PrintInfoMessage("从数据库" + dbName + "中读取:" + key
				// + "出现lock异常");
				// CheckMethods.PrintInfoMessage(lockConflict.getMessage());
				// CheckMethods.PrintInfoMessage(lockConflict.getCause().toString());
				// CheckMethods.PrintInfoMessage(lockConflict.getStackTrace().toString());
